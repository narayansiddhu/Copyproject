...
public class Csethree extends AppCompatActivity implements AdapterView.OnItemClickListener
{

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_csethree);
        ...
        for (Csethree_List enum_of_csethree : Csethree_List.values()) {
            adapter_of_titles.add(getString(enum_of_csethree.title));
        }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        Csethree_List item_of_list = Csethree_List.values()[position];
        switch (item_of_list) {
            case
                    choice_18:
                Intent intent_Csethreeonein = new Intent(Csethree.this, Csethreeonein.class);
                startActivity(intent_Csethreeonein);
                break;
            case
                    choice_19:
                Intent intent_Csethreetwoin = new Intent(Csethree.this, Csethreetwoin.class);
                startActivity(intent_Csethreetwoin);
                break;
